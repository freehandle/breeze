...
The node's public key at the "token" field for blow's configuration file must match node's owner vault public key. 

In order to sync to the running node, kite module is used from vault owner. Node's address in the DNS:port format and node's ephemeral token, automatically generated by the blow module upon creating the node with the config file, must be provided:
